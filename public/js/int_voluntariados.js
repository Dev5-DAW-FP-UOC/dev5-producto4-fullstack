// public/js/volunteers.js
// Voluntariados (P4): persistencia en Mongo (GraphQL) + Canvas + UI

import { altaVoluntariado, borrarVoluntariado, listarVoluntariados, getActiveUser, listarSeleccionados, borrarSeleccionados } from "./almacenaje.js";
...
function normType(t) {
  ...
  if (v.includes("pet")) return "petición";
  return "oferta";
}
...
/**
 * Al borrar un voluntariado, hay que borrar también las selecciones
 * que apunten a ese id_voluntariado.
 * (En P4 borrarSeleccionados() borra por id de selección, no por id_voluntariado.)
 */
async function borrarSeleccionesDeVoluntariado(volId) {
  try {
    const sels = await listarSeleccionados(); // [{id, id_usuario, id_voluntariado}]
    const toDelete = (sels || []).filter((s) => s.id_voluntariado === volId);
    for (const s of toDelete) {
      await borrarSeleccionados(s.id);
    }
  } catch (err) {
    // Si no existían selecciones o no hay permisos, no bloqueamos el borrado del voluntariado
    console.log("No fue necesario borrar selecciones asociadas o no había.");
  }
}
...
async function handleSubmit(e) {
  ...
  const nuevo = {
    ...
    type: normType(f.tipo?.value || "oferta"),
    // P4 usa sesión -> id_usuario se calcula en backend (en tu adaptador lo saca de me)
    descripcion: (f.descripcion?.value || "").trim(),
    resumen: (f.descripcion?.value || "").trim(),
    fecha: f.fecha?.value || todayISO(),
    modalidad: f.modalidad?.value || "Presencial",
    creadoPor: active?.nombre || "Anónimo", // solo para UI (backend puede ignorarlo)
    email: (f.email?.value || "").trim(), // solo para compatibilidad UI
  };
  ...
  try {
    ...
    console.error("altaVoluntariado failed:", err);
...
async function handleListClick(e) {
  ...
  await borrarSeleccionesDeVoluntariado(id);
...
async function loadFromDB() {
  // listarVoluntariados ya viene filtrado por rol desde backend:
  // - admin: todos
  // - user: solo los suyos
  state.vols = (await listarVoluntariados()).map((v) => ({
    ...v,
    creadoPor: getAutorLabel(v),
  }));
}
